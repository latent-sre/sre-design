...
description: >-
  Generate-phase alert drafter (coverage #19) — propose which parsed error/warn log lines
  ...
  or turn error logs into alerts.
...
3. Your findings become `Alert` artifacts (`source_tier: llm`) flagged for human review, rendered through the
...
finding is flagged for human review. The engine runs `sre-kb generate-alerts` to re-ground them.
