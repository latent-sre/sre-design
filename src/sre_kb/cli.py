...
def gap_finder_cmd(
    ...
    """Tier-B LLM gap-finder (SPIKE): ingest Copilot's gap proposals, re-ground each
    (locate -> stamp path:line:hash -> re-derive), and emit ResiliencyGap artifacts.

    The engine never calls a model — it ingests proposals Copilot already wrote by running
    the sre-gap-finder (assess-resiliency) skill. Nothing proposed can auto-verify.
    """
    ...
    for o in run.result.outcomes:
        ...
        typer.echo(f"  [{o.result:<12}] {o.proposal.pattern} on {o.proposal.target}{where}  — {o.note}")
